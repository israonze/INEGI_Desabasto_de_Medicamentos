...
DiabetesMx= Diabetes_PASOP18 %>%
  ...
  mutate(tipo=str_extract(tolower(descripcion_cucop), "tableta|inyectable|jarabe|gragea"),
         A= str_extract(tolower(descripcion_cucop), "\\d+\\.?\\d? mg ?/\\d+\\.?\\d? ?mg|\\d+\\.?\\d? mg|\\d+\\.?\\d? ui|\\d+\\.?\\d? ml|\\d+\\/\\d+\\.?\\d+\\d? mg|\\d+\\.?\\d? ml\\/\\d+\\.?\\d+\\d? mg"),
         B=  str_extract(tolower(producto), "\\d+\\.?\\d? mg ?/\\d+\\.?\\d? ?mg|\\d+\\.?\\d? mg|\\d+\\.?\\d?mg|\\d+\\.?\\d? ui|\\d+\\.?\\d? ml|\\d+\\/\\d+\\.?\\d+\\d? mg|\\d+\\.?\\d? ml\\/\\d+\\.?\\d+\\d? mg"),
         dosis=coalesce(A,B),
         #dosis= if_else(is.na(DiabetesMx$dosis),str_extract(tolower(producto), "\\d+\\.?\\d? mg ?/\\d+\\.?\\d? ?mg|\\d+\\.?\\d? mg|\\d+\\.?\\d? ui|\\d+\\.?\\d? ml|\\d+\\/\\d+\\.?\\d+\\d? mg|\\d+\\.?\\d? ml\\/\\d+\\.?\\d+\\d? mg")),
